...
export function FeatureFlagsPanel() {
  ...
  const [demoMode, setDemoMode] = useState(() => localStorage.getItem('rita_demo_mode') === 'true')

  const handleDemoModeToggle = (checked: boolean) => {
    localStorage.setItem('rita_demo_mode', String(checked))
    setDemoMode(checked)
    toast.success(`Demo mode ${checked ? 'enabled' : 'disabled'} - refresh to apply`)
  }
      ...
      {import.meta.env.DEV && (
        <Card className="p-6 border-blue-200 dark:border-blue-900 bg-blue-50/50 dark:bg-blue-950/20">
          ...
          <p className="text-sm text-blue-800 dark:text-blue-200 mb-4">
            Bypass authentication and use admin permissions for local development.
          </p>
          <Separator className="my-4" />
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Skip auth, show admin nav items (Home, Knowledge, Tickets)
              </p>
            </div>
            <Switch
              id="demo-mode"
