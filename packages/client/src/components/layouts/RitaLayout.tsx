...
function RitaLayoutContent({ children, activePage = "chat" }: RitaLayoutProps) {
	...
	useEffect(() => {
		// Skip in dev mode without auth
		if (import.meta.env.DEV && !profile?.user?.id) {
			return;
		}
	...
	}, [showWelcomeModal, hasSeenWelcomeModal, markWelcomeModalAsSeen, profile?.user?.id]);
							...
							{import.meta.env.DEV && (
									<SidebarMenuItem>
										<SidebarMenuButton
											...
											isActive={location.pathname === "/devtools"}
										>
